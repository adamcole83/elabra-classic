<?php
	...
	if (isset($_POST["directory"])) {
		$save_path = PUBLIC_ROOT.$_POST['directory'];
	} else if (isset($_GET["directory"])) {
		$save_path = PUBLIC_ROOT.$_GET['directory'];
	}
	$upload_name = "Filedata";
	$max_file_size_in_bytes = 524288000;					// 2MB in bytes
	$extension_whitelist = array_keys(get_allowed_mime_types());	// Allowed file extensions
...
/*
	if (file_exists($save_path . $file_name)) {
		HandleError("Filename already exists.");
		//exit(0);
	}
*/
	...
	$d = (ext2type($file_extension) == 'image') ? @getimagesize($_FILES[$upload_name]["tmp_name"]) : null;
	if(!Group::can('manage_banners', $_POST['user_id']) && ($d[0] == 550 && ($d[1] == 200 || $d[1] == 196 )))
	{
		HandleError("You are not authorized to manage banners.");
		exit(0);
	}
	...
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$file_name)) {
		...
		chmod($save_path.$file_name, 0775);
	}
